...
namespace TMPro.EditorUtilities
{
    ...
    class LigatureSubstitutionRecordPropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            int size = EditorGUI.DelayedIntField(new Rect(rect.x, position.y + 3, 130, rect.height),
                new GUIContent("Component Glyphs"), prop_ComponentGlyphIDs.arraySize);
            ...
            for (int i = 0; i < ComponentGlyphIDCount; i++)
            {
                ...
                EditorGUI.DelayedIntField(
                    new Rect(componentGlyphPosition.x - 13, componentGlyphPosition.y + 73, 40,
                        EditorGUIUtility.singleLineHeight), prop_ComponentGlyphIDs.GetArrayElementAtIndex(i),
                    GUIContent.none);
                if (EditorGUI.EndChangeCheck())
                {
                }
            ...
            Rect ligatureGlyphPosition = new Rect(50 + (glyphSpacing * ComponentGlyphIDCount + 1), position.y + 3, 95,
            ...
            DrawGlyph((uint)prop_LigatureGlyphID.intValue,
                new Rect(ligatureGlyphPosition.x + 37, ligatureGlyphPosition.y + 21, 48, 48), property);

            EditorGUI.BeginChangeCheck();
            EditorGUI.DelayedIntField(
                new Rect(ligatureGlyphPosition.x + 24, ligatureGlyphPosition.y + 94, 40,
                    EditorGUIUtility.singleLineHeight), prop_LigatureGlyphID, GUIContent.none);
            if (EditorGUI.EndChangeCheck())
            {
            }
        }
